...
class EventListenerList implements IteratorAggregate {
	...
	public function remove($idItemOrListener): void {
		...
		else if (is_string($idItemOrListener) && isset($this->itemsById[$idItemOrListener]))
			$item = $this->itemsById[$idItemOrListener];
		else if (is_callable($idItemOrListener)) {
			$list = $this->itemsById;
	...
	public function getIterator() {
		if ($this->isDirty) $this->sortItems();
	...
	protected function sortItems(): void {
		...
		while (!empty($itemsWithPivotId) && $c < $limit) {
			...
			foreach ($itemsWithPivotId as $item) {
				...
				else $itemsWithPivotIdFiltered[$item->id] = $item;
			}
			$itemsWithPivotId = $itemsWithPivotIdFiltered;
		}
		if (!empty($itemsWithPivotId))
			throw new CircularPivotIdException("You have an issue with your event's pivot id's! The pivot id's that failed are: " . implode(", ", array_keys($itemsWithPivotId)));
