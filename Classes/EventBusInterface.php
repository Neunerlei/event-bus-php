...
interface EventBusInterface extends EventDispatcherInterface
{
     ...
     *
     * @param   string[]|string  $events    Either the class name of an event to listen for or a list of events to
     *                                      listen for.
     * @param   callable         $listener  A callback which is executed when the matching event is dispatched
     * @param   array            $options   Additional options
     *                                      - priority: int (0) Can be used to define the order of handlers when bound
     *                                      on the same event. 0 is the default the "+ range" is a higher priority
     *                                      (earlier) the "- range" is a lower priority (later)
     *                                      - id: string (GENERATED) The identifier by which this listener should be
     *                                      known. If not specified one will be generated. If the id was not given, it
     *                                      will be present after the event was bound.
     ...
     *                                      listener is only guaranteed to come before the specified existing listener.
     *                                      No guarantee is made regarding when it comes relative to any other
     *                                      listener.
     ...
     *
     * @param   EventSubscriberInterface  $instance  The instance that wants to subscribe it's methods as event
     *                                               listeners
     *
     * @return $this
